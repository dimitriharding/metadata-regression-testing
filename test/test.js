...
test('Should remove diff', async t => {
	removeTestData();

	await mrt({}, GITHUB_URL);
	await mrt({}, GOOGLE_URL);
	const diffPresentBefore = fs.existsSync(diffFilePath);

	await mrt({}, GITHUB_URL);
	const diffPresentAfter = fs.existsSync(diffFilePath);

	t.plan(2);
	t.true(diffPresentBefore);
	t.false(diffPresentAfter);
});
